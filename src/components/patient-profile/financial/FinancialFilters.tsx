"use client";

import { Filter, Search } from "lucide-react";
import type { ChargeStatus } from "@/lib/financial-types";
import { PAYMENT_METHOD_LABELS } from "@/lib/financial-mock";
import type { PaymentMethodType } from "@/lib/budget-types";

export type FinancialFilterState = {
  search: string;
  status: ChargeStatus | "todos";
  paymentMethod: PaymentMethodType | "todos";
  type: "todos" | "parcela" | "avista";
  period: "todos" | "30d" | "90d" | "365d";
};

export const DEFAULT_FINANCIAL_FILTERS: FinancialFilterState = {
  search: "",
  status: "todos",
  paymentMethod: "todos",
  type: "todos",
  period: "todos",
};

export function FinancialFilters({
  filters,
  onChange,
}: {
  filters: FinancialFilterState;
  onChange: (patch: Partial<FinancialFilterState>) => void;
}) {
  return (
    <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm lg:flex-row lg:items-center">
      <label className="relative min-w-0 flex-1">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
        <input
          type="search"
          value={filters.search}
          onChange={(e) => onChange({ search: e.target.value })}
          placeholder="Buscar título ou orçamento..."
          className="w-full rounded-xl border border-slate-200 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-500/15"
        />
      </label>

      <select
        value={filters.status}
        onChange={(e) => onChange({ status: e.target.value as FinancialFilterState["status"] })}
        className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm lg:w-36"
      >
        <option value="todos">Status: Todos</option>
        <option value="pendente">Pendente</option>
        <option value="pago">Pago</option>
        <option value="vencido">Vencido</option>
        <option value="agendado">Agendado</option>
        <option value="cancelado">Cancelado</option>
      </select>

      <select
        value={filters.paymentMethod}
        onChange={(e) =>
          onChange({ paymentMethod: e.target.value as FinancialFilterState["paymentMethod"] })
        }
        className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm lg:w-40"
      >
        <option value="todos">Pagamento: Todos</option>
        {(Object.keys(PAYMENT_METHOD_LABELS) as PaymentMethodType[]).map((k) => (
          <option key={k} value={k}>
            {PAYMENT_METHOD_LABELS[k]}
          </option>
        ))}
      </select>

      <select
        value={filters.type}
        onChange={(e) => onChange({ type: e.target.value as FinancialFilterState["type"] })}
        className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm lg:w-32"
      >
        <option value="todos">Tipo: Todos</option>
        <option value="parcela">Parcela</option>
        <option value="avista">À vista</option>
      </select>

      <select
        value={filters.period}
        onChange={(e) => onChange({ period: e.target.value as FinancialFilterState["period"] })}
        className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm lg:w-36"
      >
        <option value="todos">Período: Todos</option>
        <option value="30d">30 dias</option>
        <option value="90d">90 dias</option>
        <option value="365d">12 meses</option>
      </select>

      <button
        type="button"
        className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700"
      >
        <Filter className="h-4 w-4" />
        Filtros
      </button>
    </div>
  );
}
