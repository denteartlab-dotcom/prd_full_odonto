...
export function BudgetActionsMenu({
  ...
  const items = [
    ...
    { icon: FileDown, label: "Gerar PDF", onClick: onPdf },
    { icon: MessageCircle, label: "Enviar WhatsApp", onClick: onWhatsApp ?? (() => {}) },
    { icon: Receipt, label: "Gerar cobrança", onClick: onCharge ?? (() => {}) },
    ...
    open && mounted
      ? createPortal(
          <>
            ...
            <div
              ref={menuRef}
              className="fixed z-[9999] w-44 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl"
